            ...
            <form method="POST" action="{{ route('admin.screening.questions.update', $question) }}" class="space-y-6">
                ...
                <div><label class="block text-sm font-medium mb-2">Tipe Jawaban *</label>
                    <select name="tipe_jawaban" id="tipe_jawaban" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-red-500" required>
                        <option value="ya_tidak" {{ old('tipe_jawaban', $question->tipe_jawaban) == 'ya_tidak' ? 'selected' : '' }}>Ya/Tidak</option>
                        <option value="pilihan_ganda" {{ old('tipe_jawaban', $question->tipe_jawaban) == 'pilihan_ganda' ? 'selected' : '' }}>Pilihan Ganda</option>
...
document.getElementById('tipe_jawaban').addEventListener('change', function() {
    ...
    pilihanContainer.classList.toggle('hidden', this.value !== 'pilihan_ganda');
});
